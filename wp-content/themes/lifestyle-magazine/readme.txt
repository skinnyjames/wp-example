=== lifestyle-magazine ===
Contributors: thebootstrapthemes
Tags: right-sidebar, portfolio, e-commerce, theme-options, featured-images, rtl-language-support, grid-layout, editor-style, custom-header, threaded-comments, custom-background, custom-colors, blog, footer-widgets
Requires at least: 4.5
Tested up to: 4.9.5
Stable tag: 1.1.3
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

lifestyle-magazine is distributed under the terms of the GNU General Public License v2

Description: Lifestyle Magazine WordPress Theme is a premium WordPress theme made focusing on travel & tour operators.  This theme is based on WP Travel Engine WordPress Plugin. This theme is very lightweight, easy to use, SEO friendly and customizable according to your necessity and desire. The theme is build in Bootstrap Framework. This is one of the best WordPress theme designed for travel blogging, trekking, lifestyle, agency, travel booking, tour operators, bloggers & blogging, bed and breakfast, hotel reservation, tourism, lifestyle, vacation, holidays, ticketing, adventure, small medium and large scale organisations etc.


== Installation ==
	
1. In your admin panel, go to Appearance > Themes and click the Add New button.
2. Click Upload and Choose File, then select the lifestyle-magazine.zip file. Click Install Now.
3. Click Activate to use your new theme right away.
4. Go to Customiser -> Home Page Setting -> Select - Static Front Page
5. Create categories, pages and post
6. Select category needed for the homepage

Lifestyle Magazine WordPress Theme, Copyright 2018 The Bootstrap Themes
Lifestyle Magazine is distributed under the terms of the GNU GPL




== Changelog ==

	= Version 1.1.3 =
		Released Date: Jun 4, 2020
			i) front-page.php removed.

	= Version 1.1.2 =
		Released Date: Jun 3, 2020
			i) TBTthemes-demoimport added as recommded for Demo import
			ii) Demo import feature added via plugin.
			iii) Recommended plugins modified.

	= Version 1.1.1 =
		Release Date: May 18, 2020
			i) Remove demo file from the folder
			ii readme.txt versioning fixed.

	= Version 1.1.0 =
		Release Date: Dec 16, 2019

	= Version 1.0.5 =
		Release Date: Jan 17, 2019

	= Version 1.0.4 =
		Release Date: Jan 15, 2019

	= Version 1.0.3 =
		Release Date: Nov 23, 2019

	= Version 1.0.2 =
		Release Date: Nov 5, 2019

	= Version 1.0.1 =
		Release Date: October 26, 2019

	= Version 1.0.0 =
		Release Date: October 25, 2018
			i) Intial version Release




== Credits ==

* Based on Underscores http://underscores.me/, (C) 2012-2016 Automattic, Inc., [GPLv2 or later](https://www.gnu.org/licenses/gpl-2.0.html)
* normalize.css http://necolas.github.io/normalize.css/, (C) 2012-2015 Nicolas Gallagher and Jonathan Neal, [MIT](http://opensource.org/licenses/MIT)
* Font-Awesome: fonts, License: SIL OFL 1.1
* Font-Awesome: css, License: MIT License
* Bootstrap: http://getbootstrap.com, License: MIT License
* Google Fonts - Apache License, version 2.0
* wp-bootstrap-navwalker – ​https://github.com/twittem/wp-bootstrap-navwalker, License: Distributed under the terms of the GPL-2.0+ @twittem



* Image licences Used in screenshot 
- https://pixabay.com/en/woman-adult-portrait-people-girl-3083396/
License: Creative Commons CC0 - Free for commercial use.

* Image licences Used in screenshot 
- https://pixabay.com/en/girl-fashion-portrait-fashion-girl-2143709/
License: Creative Commons CC0 - Free for commercial use.

* Image licences Used in screenshot 
- https://pixabay.com/en/girl-model-pink-fashion-portrait-2189248/
License: Creative Commons CC0 - Free for commercial use.

* Image licences Used in screenshot 
- https://pixabay.com/en/beach-woman-casual-enjoyment-2178919/
License: Creative Commons CC0 - Free for commercial use.

* Image licences Used in screenshot 
- https://pixabay.com/en/woman-sunglasses-eyewear-brunette-1246299/
License: Creative Commons CC0 - Free for commercial use.

* Image licences Used in screenshot 
- https://pixabay.com/en/beautiful-car-driver-face-fashion-1845572/
License: Creative Commons CC0 - Free for commercial use.

* Image licences Used in screenshot 
- https://pixabay.com/en/attractive-beautiful-cute-face-1846127/
License: Creative Commons CC0 - Free for commercial use.

* Image licences Used in screenshot 
- https://pixabay.com/en/beautiful-sun-hat-sun-protection-1845148/
License: Creative Commons CC0 - Free for commercial use.


* trt-customizer-pro https://github.com/justintadlock/trt-customizer-pro, (C) 2016 © Justin Tadlock,  GNU GPL version 2 or later

* Wow.js: https://github.com/matthieua/WOW , License: MIT License
* Animate.css: https://github.com/daneden/animate.css , License: MIT License
* Owl-carousel: https://owlcarousel2.github.io/OwlCarousel2, Copyright 2013-2017 David Deutsch , License : MIT

* Font-Awesome: https://github.com/FortAwesome/Font-Awesome FontAwesome 4.6.3 Copyright 2012 Dave Gandy Font License: SIL OFL 1.1 Code License: MIT License http://fontawesome.io/license/